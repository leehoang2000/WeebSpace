...
func get_position()-> Vector2:
	...
	if last_position == null or last_position == player_position:
		...
		return player_position * rng.randf_range(3,5) + random_vector_within_range()
	else:
		var displacement_vector = player_position - last_position
		last_position = player_position
		return (player_position 
				+ displacement_vector * 64
				+ rng.randf_range(-32, 32) * Vector2(displacement_vector.y, (-1)*displacement_vector.x)
